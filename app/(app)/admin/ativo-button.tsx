      ...
      <button
        ...
        className={`rounded-md border px-3 py-1 text-xs font-medium disabled:opacity-50 ${
          ativo
            ...
            : "border-chumbo/20 text-zinc-600 hover:bg-brand hover:text-chumbo dark:border-white/20 dark:text-zinc-300"
        }`}
      >
        {isPending ? "Salvando..." : ativo ? "Desativar conta" : "Reativar conta"}
      </button>
      {error && <p className="max-w-[220px] text-right text-xs text-red-600 dark:text-red-400">{error}</p>}
    </div>
  );
}
